...
import { Interface, Method, PropDefinition, PropDefinitions, RequestBodyType, RequestFormItemType, Required, ResponseBodyType } from '../types';
 ...
 * @returns {boolean}
 */
export function hasPackageJson(dirPath: string) {
...
export function traverseJsonSchema(
  ...
  if (isArray(jsonSchema.properties)) {
    jsonSchema.properties = (jsonSchema.properties as JSONSchema4[]).reduce<Defined<JSONSchema4['properties']>>((props, js) => {
      props[js.name] = js;
      return props;
    }, {});
  }
 ...
 */
export function processJsonSchema(jsonSchema: JSONSchema4, customTypeMapping: Record<string, JSONSchema4TypeName>): JSONSchema4 {
 ...
 */
export function jsonSchemaStringToJsonSchema(str: string, customTypeMapping: Record<string, JSONSchema4TypeName>): JSONSchema4 {
 ...
 */
export function propDefinitionsToJsonSchema(propDefinitions: PropDefinitions, customTypeMapping: Record<string, JSONSchema4TypeName>): JSONSchema4 {
 ...
 * @returns JSONSchema 对象
 */
export function jsonToJsonSchema(json: object, customTypeMapping: Record<string, JSONSchema4TypeName>): JSONSchema4 {
 ...
 * @returns JSONSchema 对象
 */
export function mockjsTemplateToJsonSchema(template: object, customTypeMapping: Record<string, JSONSchema4TypeName>): JSONSchema4 {
...
export function getRequestDataJsonSchema(interfaceInfo: Interface, customTypeMapping: Record<string, JSONSchema4TypeName>): JSONSchema4 {
...
export function reachJsonSchema(jsonSchema: JSONSchema4, path: OneOrMore<string>) {
  let last = jsonSchema;
  ...
  return traverseJsonSchema(jsonSchema, (jsonSchema, currentPath) => {
    ...
    if (refValue?.startsWith('&')) {
      ...
      const typeAbsolutePath = toUnixPath(
        path.resolve(path.dirname(`/${currentPath.join('/')}`.replace(/\/{2,}/g, '/')), typeRelativePath).replace(/^[a-z]+:/i, '')
      );
...
export async function jsonSchemaToType(jsonSchema: JSONSchema4, typeName: string): Promise<string> {
  ...
  const code = await compile(jsonSchemaToJSTTJsonSchema(cloneDeepFast(jsonSchema), typeName), fakeTypeName, JSTTOptions);
